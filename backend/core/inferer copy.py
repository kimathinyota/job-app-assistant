...
class JobDescriptionParser:
    def __init__(self, model_path: str, context_size: int = 4096, n_threads: int = 4):
        if Llama is None:
...
"""
    ...
    def _run_inference(self, prompt_template: str, doc_text: str, max_tokens: int = 1024, doc_label: str = "JOB DESCRIPTION") -> dict:
        """
...
"""
    ...
    def fast_parse_cv(self, cv_text: str, cv_name: str = "Imported CV") -> CV:
        ...
        REMOVED: 'Output:' injection (to fix confusion).
        INCLUDES: Data Sanitization (to fix Pydantic crash).
        """
        log.info("🚀 Starting Fast CV Parse...")
        
        # 1. Run Inference (Cleaned up calls)
        # We removed the ' + "\nOutput:"' from these calls
        identity_data = self._run_inference(self.PROMPT_CV_IDENTITY, cv_text, max_tokens=1000, doc_label="CANDIDATE RESUME")
        exp_data = self._run_inference(self.PROMPT_CV_EXPERIENCE, cv_text, max_tokens=2500, doc_label="CANDIDATE RESUME")
        edu_data = self._run_inference(self.PROMPT_CV_EDUCATION, cv_text, max_tokens=1000, doc_label="CANDIDATE RESUME")
        proj_data = self._run_inference(self.PROMPT_CV_PROJECTS, cv_text, max_tokens=1000, doc_label="CANDIDATE RESUME")

        # 2. Sanitize Contact Info (Prevents Pydantic Crash)
        raw_contact = {
        ...
        # 4. Helper for Linking/Deduping Skills
        skill_registry = {} 
        ...
        for item in exp_data.get("items", []) or []:
            ...
            if item.get("achievements_list"):
                for ach in str(item.get("achievements_list", "")).split(';'):
                    if len(ach) > 3:
                        cv_obj.add_achievement(text=ach.strip(), context=item.get("company"))\
                              .link_to_experience(exp.id)
        ...
        for item in edu_data.get("items", []) or []:
            ...
            if item.get("details_list"):
                for det in str(item.get("details_list", "")).split(';'):
                     if len(det) > 3:
                        cv_obj.add_achievement(text=det.strip(), context=item.get("institution"))\
                              .link_to_education(edu.id)
        ...
        for item in proj_data.get("items", []) or []:
            ...
            if item.get("achievements_list"):
                for ach in str(item.get("achievements_list", "")).split(';'):
                    if len(ach) > 3:
                        cv_obj.add_achievement(text=ach.strip(), context="Project")\
                              .link_to_project(proj.id)
...
"""
    ...
    def fast_parse_cv(self, cv_text: str, cv_name: str = "Imported CV") -> CV:
        ...
        identity_data = self._run_inference(self.PROMPT_CV_IDENTITY + "\nOutput:", cv_text)
        exp_data = self._run_inference(self.PROMPT_CV_EXPERIENCE + "\nOutput:", cv_text)
        edu_data = self._run_inference(self.PROMPT_CV_EDUCATION + "\nOutput:", cv_text)
        proj_data = self._run_inference(self.PROMPT_CV_PROJECTS + "\nOutput:", cv_text)
