...
@router.get("/{work_id}")
def get_work_item(work_id: str, request: Request):
...
def update_work_item(work_id: str, data: WorkItemUpdate, request: Request):
    """Update a work item."""
    try:
        registry = request.app.state.registry
...
def delete_work_item(work_id: str, request: Request):
...
def mark_work_item_complete(work_id: str, request: Request, reflection: Optional[str] = None):
