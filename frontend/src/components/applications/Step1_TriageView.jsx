import React, { useState, useMemo, useEffect, useCallback, useRef } from 'react'; // Imports are correct
import { 
    Wand2, Check, X, Trash2, Eye, Edit3, ArrowRight, 
...
// --- CONSTANTS ---
// ... (Tuning Modes and Triage Groups are unchanged) ...
const TUNING_MODES = {
...
// ... (LoadingOverlay component is unchanged) ...
...
// ... (TriageModeView component is unchanged) ...
const TriageModeView = ({ 
    ...
    return (
        <div className="animate-fade-in">
            {/* --- FIXED: Category Tabs (Now Responsive) --- */}
...
const ManualModeView = ({ 
    ...
    // ... (State and other hooks are unchanged) ...
    ...
    // --- FIX: Refs for search inputs to control focus ---
    const reqSearchInputRef = useRef(null);
    const evidenceSearchInputRef = useRef(null);

    // --- FIX: useEffects to manually focus input *only* when opened ---
    useEffect(() => {
        if (showReqSearch && reqSearchInputRef.current) {
            reqSearchInputRef.current.focus();
        }
    }, [showReqSearch]);

    useEffect(() => {
        if (showEvidenceSearch && evidenceSearchInputRef.current) {
            evidenceSearchInputRef.current.focus();
        }
    }, [showEvidenceSearch]);


    // ... (Memo logic for pairs/suggestions is unchanged) ...
    ...
    // --- FIXED: Wrapped handleManualClear in useCallback ---
    const handleManualClear = useCallback(() => {
        setSelectedReqId(null);
        setSelectedContextId(null);
        setSelectedContextType(null);
        setAnnotation("");
        setMobileStep(1); // Reset mobile view
    }, []); // No dependencies, this function is stable

    // --- NEW: Effect to clear state on breakpoint change ---
    useEffect(() => {
        // Bootstrap's 'lg' breakpoint is 992px. We listen for changes *across* this breakpoint.
        const mql = window.matchMedia("(max-width: 991.98px)");

        const handleChange = (e) => {
            if (e.matches) {
                // We just *entered* mobile view (d-lg-none is now visible)
                // Clear selections to reset the wizard
                handleManualClear();
            }
        };
    ...
    }, [handleManualClear]); // Depend on the stable handleManualClear function
    ...
    const filteredPairs = useMemo(() => {
        ...
        pairs = pairs.filter(pair => {
            ...
            const matchesSearch = (
                (pair.feature_text || '').toLowerCase().includes(pairSearch.toLowerCase()) ||
    ...
    const handleTablePreview = (pair) => {
        ...
        if (lookup && lookup.item) {
            onPreview(lookup.item, pair.context_item_type);
        }
    };

    // ... (List filtering logic is unchanged) ...
    const filteredFeatures = useMemo(() => {
        return (job.features || []).filter(req => req.description.toLowerCase().includes(reqSearch.toLowerCase()));
    }, [job.features, reqSearch]);

    const filteredEvidence = useMemo(() => {
        return cvEvidenceList.filter(item => item.text.toLowerCase().includes(evidenceSearch.toLowerCase()));
    }, [cvEvidenceList, evidenceSearch]);

    // --- Helper components for mobile/desktop steps ---
        ...
        <div className="card shadow-sm h-100 border-0 rounded-4 overflow-hidden">
            ...
            <div className="list-group list-group-flush overflow-auto custom-scroll" style={{height: '400px'}}>
                {filteredFeatures.map(req => {
                    ...
                    let itemClass = 'bg-white text-dark';
                    if (isSelected) itemClass = 'bg-primary text-white border-primary';
                    else if (isPaired) itemClass = 'bg-light text-muted fst-italic';
                    else if (isSuggested) itemClass = 'bg-primary-subtle text-primary-emphasis fw-medium border-primary-subtle'; 

                    return (
                        <button key={req.id} 
                            className={`list-group-item list-group-item-action border-0 border-bottom small p-3 transition-all ${itemClass} ${isDisabled ? 'opacity-50' : ''}`}
                            onClick={() => setSelectedReqId(prev => {
                                const newId = prev === req.id ? null : req.id;
                                if (isMobile && newId) setMobileStep(2); // Advance on mobile
                                return newId;
                            })}
                            disabled={isDisabled}
                        >
    ...
    const EvidenceColumn = ({ isMobile = false, inputRef }) => (
        <div className="card shadow-sm h-100 border-0 rounded-4 overflow-hidden">
            <div className="card-header bg-success bg-opacity-10 text-success fw-bold small border-bottom p-3 d-flex justify-content-between align-items-center">
                {isMobile && (
                    <button className="btn btn-sm p-0 text-success me-2" onClick={() => setMobileStep(1)}>
                        <ArrowLeft size={18} />
                    </button>
                )}
                <span className="flex-grow-1">2. Select Evidence</span>
                <button className={`btn btn-sm p-0 ${showEvidenceSearch ? 'text-success' : 'text-muted hover-text-success'}`} onClick={() => setShowEvidenceSearch(!showEvidenceSearch)}>
                    <Search size={14}/>
                </button>
            </div>
            {showEvidenceSearch && (
                        ...
                        <button key={item.id} 
                            ...
                            onClick={() => {
                                if (isSelected) { 
                                    ...
                                    if (isMobile) setMobileStep(3); // Advance on mobile
                                }
                            }}
                            disabled={isDisabled}
                        >
                            <div className="d-flex justify-content-between align-items-center">
                                <div className="d-flex flex-column align-items-start text-start overflow-hidden">
                                    ...
                                    <span className="badge bg-white bg-opacity-25 border border-white border-opacity-25 fw-normal" style={{fontSize: '0.65rem'}}>
                                ...
                                <div className="d-flex gap-2 align-items-center ms-2">
                                    {isPaired && <Link2 size={14}/>}
                                    {isSuggested && <Wand2 size={14} className="animate-pulse"/>}
                                    <div role="button" onClick={(e) => { e.stopPropagation(); onPreview(item.item, item.type); }} className="p-1 rounded hover-bg-white-25">
                                        <Eye size={14}/>
                                    </div>
                                </div>
                            </div>
    ...
    const ConnectColumn = ({ isMobile = false, reqText, evidenceItem, onPreview }) => (
        <div className="card shadow-sm h-100 border-0 rounded-4 overflow-hidden">
            <div className="card-header bg-dark bg-opacity-10 text-dark fw-bold small border-bottom p-3 d-flex align-items-center gap-2">
                {isMobile && (
                    <button className="btn btn-sm p-0 text-dark me-2" onClick={() => setMobileStep(2)}>
                        <ArrowLeft size={18} />
                    </button>
                )}
            ...
            <div className="card-body d-flex flex-column p-4 bg-white h-100">
                ...
                <div className="mb-3">
                    ...
                    {(!reqText && !evidenceItem) ? (
                        <div className="text-center py-4 bg-light rounded-3 border-dashed">
                            <p className="small text-muted fst-italic mb-0">Select items to begin</p>
                        </div>
                    ) : (
                        <div className="d-flex flex-column gap-2">
                            {reqText && (
                                <div className="d-flex align-items-start p-3 bg-white rounded-3 border border-primary-subtle shadow-sm">
                                    ...
                                    <span className="small fw-medium text-dark text-wrap" title={reqText}>
                                        {reqText}
                                    </span>
                                </div>
                            )}
                            {evidenceItem && (
                                <div className="d-flex justify-content-between align-items-start p-3 bg-white rounded-3 border border-success-subtle shadow-sm">
                                    <div className="d-flex align-items-start me-2" style={{minWidth: 0}}>
                                        ...
                                        <span className="small fw-medium text-dark text-wrap" title={evidenceItem.text}>
                                            {evidenceItem.text}
                                        </span>
                                    </div>
                                    <button 
                                        className="btn btn-sm btn-link text-success p-0 flex-shrink-0 ms-auto"
                                        onClick={() => onPreview(evidenceItem.item, evidenceItem.type)}
                ...
                <div className="mt-auto"> 
                    ...
                    <textarea className="form-control bg-light border-0 shadow-sm text-dark mb-3" 
                        placeholder={selectedReqId && selectedContextId ? "Add your reasoning..." : "Select items to add reasoning"}
                        value={annotation}
                    ...
                    {(selectedReqId || selectedContextId) && (
                        <button className="btn btn-sm btn-outline-secondary w-100 mb-3" onClick={handleManualClear}>
                            Clear Selection
                        </button>
                    )}

                    <button className={`btn w-100 py-3 fw-bold shadow-sm ${isAlreadyPaired ? 'btn-secondary' : 'btn-primary'}`}
        ...
        <div className="animate-fade-in">
            ...
            <div className="d-none d-lg-flex row g-4 mb-5">
                <div className="col-lg-4">
                    <RequirementColumn isMobile={false} inputRef={reqSearchInputRef} />
                </div>
                <div className="col-lg-4">
                    <EvidenceColumn isMobile={false} inputRef={evidenceSearchInputRef} />
                </div>
                <div className="col-lg-4">
                    <ConnectColumn 
                        isMobile={false} 
                        reqText={selectedReqText} 
            ...
            {/* --- Mobile Stepper Layout (Hidden on desktop) --- */}
                ...
                <div className="card-header bg-white py-3 px-4 border-bottom">
                    <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <h6 className="fw-bold mb-0 text-dark d-flex align-items-center gap-2">
                            <GitMerge size={18} className="text-primary"/> Mapped Pairs 
                            <span className="badge bg-light text-dark border ms-1 rounded-pill">{filteredPairs.length}</span>
                        </h6>
                        ...
                        <div className="d-flex gap-2 flex-column flex-sm-row">
                            ...
                            <div className="position-relative" style={{minWidth: '160px'}}>
                                ...
                                <select 
                                    className="form-select form-select-sm rounded-pill border-0 bg-light ps-5 pe-4 cursor-pointer" 
                                    value={filter}
                ...
                <div className="card-body p-0 custom-scroll custom-scroll-x" style={{maxHeight: '400px', overflowY: 'auto', overflowX: 'auto'}}>
                    <table className="table table-hover mb-0 align-middle">
                        <thead className="bg-light text-muted small text-uppercase sticky-top" style={{zIndex: 5}}>
                            <tr>
                                <th className="ps-4 py-3 border-0 fw-bold cursor-pointer" style={{width: '35%'}} onClick={() => requestSort('feature_text')}>
                                    Requirement <ArrowUpDown size={12} className="ms-1 opacity-50"/>
                                </th>
                                <th className="py-3 border-0 fw-bold cursor-pointer" style={{width: '30%'}} onClick={() => requestSort('context_item_text')}>
                                    CV Evidence <ArrowUpDown size={12} className="ms-1 opacity-50"/>
                                </th>
                        ...
                        <tbody className="border-top-0">
                                ...
                                return (
                                    <tr key={pair.id} className="transition-all">
                                        <td className="ps-4 py-3">
                                            <div className="text-wrap small text-dark" style={{lineHeight: '1.5', wordBreak: 'break-word'}}>{pair.feature_text}</div>
                                        </td>
                                        
                                        {/* --- FIXED: Removed hover eye, made cell clickable, and ensured text wraps --- */}
                                        <td className="py-3" style={{cursor: 'pointer'}} onClick={() => handleTablePreview(pair)}>
                                            <div 
                                                className="d-flex align-items-center gap-2"
                                                title="Click to preview evidence"
                                            >
                                                {pair.context_item_type && (
                                                    <span className="badge bg-white text-secondary border fw-normal" style={{fontSize: '0.65rem'}}>
                                                        {pair.context_item_type.slice(0,3).toUpperCase()}
                                                    </span>
                                                )}
                                                <span className="text-wrap fw-medium text-primary small text-decoration-underline-hover">
                                                    {displayEvidence}
                                                </span>
                                                {/* <Eye size={14} className="text-primary opacity-0 group-hover-opacity-100 transition-all"/> */}
                                            </div>
                                        </td>
                                        
                                        <td className="py-3">
                                            <div className="d-flex align-items-center gap-2 text-muted">
                                                {pair.annotation && <MessageSquare size={14} className="opacity-50 flex-shrink-0"/>}
                                                <span className="text-wrap fst-italic small opacity-75" style={{maxWidth: '250px'}}>{pair.annotation || '-'}</span>
                                            </div>
                                        </td>
                                        <td className="text-end pe-4 py-3">
                                            <button 
                                                className="btn btn-icon btn-sm text-danger bg-danger bg-opacity-10 hover-bg-danger hover-text-white rounded-circle transition-all" 
                            ...
                            {filteredPairs.length === 0 && (
                                <tr>
                                    <td colSpan="4" className="text-center py-5 text-muted fst-italic bg-light bg-opacity-25">
                                        <div className="mb-2"><Search size={24} className="opacity-25"/></div>
...
// --- HELPER ITEMS ---
// ... (SuggestionItem component is unchanged) ...
...
// ... (MappedItem component is unchanged) ...
...
// --- MAIN COMPONENT ---
// ... (Main Step1_TriageView component logic is unchanged) ...
const Step1_TriageView = ({ job, cv, mapping, onMappingChanged, onNext }) => {
    ...
    }, [tuningMode, mapping?.id]);

    // ... (Helpers and Memos are unchanged) ...
    const cvItemLookups = useMemo(() => {
        const lookups = new Map();
        (cv?.experiences || []).forEach(i => lookups.set(i.id, { item: i, type: 'experiences', name: `${i.title} @ ${i.company}` }));
        (cv?.projects || []).forEach(i => lookups.set(i.id, { item: i, type: 'projects', name: i.title }));
        (cv?.education || []).forEach(i => lookups.set(i.id, { item: i, type: 'education', name: `${i.degree} @ ${i.institution}` }));
        (cv?.hobbies || []).forEach(i => lookups.set(i.id, { item: i, type: 'hobbies', name: i.name }));
        return lookups;
    }, [cv]);

    const groupItems = (items, itemKey) => {
    ...
    const acceptedPairs = mapping?.pairs || [];
    ...
    const groupedSuggestions = useMemo(() => groupItems(suggestionsToReview, 'suggestions'), [suggestionsToReview, cvItemLookups]);
    const groupedPairs = useMemo(() => groupItems(acceptedPairs, 'pairs'), [acceptedPairs, cvItemLookups]);

    const cvEvidenceList = useMemo(() => [
        ...(cv?.experiences || []).map(i => ({ id: i.id, type: 'experiences', text: `${i.title} @ ${i.company}`, item: i })),
        ...(cv?.projects || []).map(i => ({ id: i.id, type: 'projects', text: i.title, item: i })),
        ...(cv?.education || []).map(i => ({ id: i.id, type: 'education', text: `${i.degree} @ ${i.institution}`, item: i })),
        ...(cv?.hobbies || []).map(i => ({ id: i.id, type: 'hobbies', text: i.name, item: i })),
    ], [cv]);
    ...
    const handleAccept = async (sugg, note) => {
        ...
        try {
            await addMappingPair(mapping.id, sugg.feature_id, sugg.context_item_id, sugg.context_item_type, note, sugg.feature_text, sugg.context_item_text);
            onMappingChanged();
    ...
    const handleDelete = async (id) => {
        setIsDeletingId(id);
        try { await deleteMappingPair(mapping.id, id); onMappingChanged(); } 
        catch (e) { alert(e.message); } 
    ...
    const handleManualAdd = async (reqId, ctxId, ctxType, note, sugg) => {
        try {
            await addMappingPair(mapping.id, reqId, ctxId, ctxType, note, sugg?.feature_text, sugg?.context_item_text);
            onMappingChanged();
        } catch (e) { alert(e.message); }
    };
    ...
    return (
        <div className="position-relative" style={{minHeight: '600px'}}>
            <style>{`
                ...
                /* Ensures the blue suggestion border is visible */
            ...
            <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center mb-5 gap-4">
                <div className="d-flex align-items-center">
                    <div>
                        ...
                        <p className="text-muted mb-0">Link your career history to job requirements.</p>
                    </div>
                </div>
            ...
            <div className="d-flex justify-content-end mt-5 pt-4 border-top">
                <button className="btn btn-outline-primary px-4 shadow-sm d-flex align-items-center gap-2" onClick={onNext}>
