...
// (Formerly Step3_ActiveCoverLetter)
...
//  import client from '../../api/client.js'; // Generic client for the rename patch
...
// Re-use your existing SectionDivider
...
const SupportingDocStudio = ({
    documentId, // The specific doc we are editing
    job,
    mapping,
    fullCV,
    isLocked = false, // READ ONLY MODE
    onBack
}) => {
    ...
    const { ideaMap, pairMap } = useMemo(() => {
        const iMap = new Map(doc?.ideas.map(i => [i.id, i]));
    ...
    }, [doc?.ideas, mapping?.pairs, cvLookups]);
    ...
    // ... (Reuse handleAddArgument, handleGlobalUpdate, handleDragEnd from previous code, adding `if(isLocked) return;` checks)
    ...
    if (!doc) return <div className="alert alert-warning">Initializing...</div>;
                            ...
                            <React.Fragment key={para.id}>
                                <div className="mb-0">
                                    <ParagraphStudio
                                        ...
                                        onAddArgument={(pid, classif) => !isLocked && handleAddArgument(pid, classif)} 
                                        onDeleteIdea={async (idea, p) => !isLocked && handleDeleteIdea(idea, p)}
                                        onRevertIdea={async (id) => !isLocked && handleRevertIdea(id)}
